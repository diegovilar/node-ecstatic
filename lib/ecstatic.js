  ...
  return function middleware (req, res, next) {
    ...
    function serve(stat) {
      ...
      if (cache) {
          ...
          if (req.headers && (new Date(Date.parse(req.headers['if-modified-since'])) >= stat.mtime)) {
              return status[304](res, next);
          }
      }
        else {
          res.setHeader('Expires', "Thu, 19 Apr 1982 00:00:00 GMT");
          res.setHeader('Cache-Control', "max-age=0, private, no-store, no-cache, must-revalidate, proxy-revalidate");
      }
